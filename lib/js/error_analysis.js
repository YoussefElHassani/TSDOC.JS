...
const {
  serializeError,
  deserializeError
} = require('serialize-error'); // adding layout
...
(function (sandbox) {
  function MyAnalysis() {
    ...
    this.scriptExit = function (iid, wrappedExceptionVal) {
      ...
      location = J$.iidToLocation(J$.sid, iid); // Get the path from the location

      filename = location.split(':')[0];
      filename = filename.replace("(", ""); // Log filename and the captured exception 

      if (wrappedExceptionVal === undefined) {
        if (!filename.includes("node_modules")) {
          infoLogger.info({
            'file': filename,
            'wrappedExceptionVal': null,
            'flag': "Success"
          });
          log4js.shutdown(() => {});
        }
        ...
        infoLogger.info({
          'file': filename,
          'wrappedExceptionVal': JSON.stringify(error),
          'flag': "Error"
        });
        log4js.shutdown(() => {});
      }
